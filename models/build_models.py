...
def build_models(data , model= 'xgb', tune= False, cv = 5, target_='30', test_size= .1):
    ...
    if model=='XGB':
        ...
        for fold_, (train_ind, val_ind) in enumerate(folds.split(train[feats], target)):
            ...
            xgb_scores.append(roc_auc_score(target30.iloc[val_ind], oof_xgb[val_ind]))
        ...
        # create model
            ...
            history = model.fit(trn_x, trn_y, validation_data=(val_x, val_y),callbacks= [cp, es, rlr], class_weight=class_weight, epochs=100, batch_size=16, verbose=1)
